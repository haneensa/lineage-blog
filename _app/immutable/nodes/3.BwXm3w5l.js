import{f as g,a as m}from"../chunks/Kp7sLg-y.js";import"../chunks/CpO_WCtV.js";import{t as b,s,h as e,i as a}from"../chunks/DxiQdkCR.js";import{s as n}from"../chunks/DvSqlD5E.js";import{b as r}from"../chunks/BM43ke9o.js";var f=g(`<main><h1>Part A: What-ifs? Evaluating Many Provenance Polynomials Fast</h1> <p>Lineage isn’t just about knowing which inputs produced which outputs. It’s also a powerful tool for <em>what-if analysis</em>.  
Imagine asking: <strong>“What if we removed all sensitive orders?”</strong> or <strong>“What if we scaled certain orders by 2x?”</strong> With fine-grained lineage, we can evaluate these hypothetical updates quickly, without recomputing the full query from scratch.</p> <h2>From Provenance Polynomials to Hypotheticals</h2> <p>Recall our earlier example with <code>customer</code> and <code>orders</code>:</p> <pre><code>Hannah (oid=1): c1*o1 + c1*o2 + c1*o4  
Alex   (oid=2): c2*o3  
...
FROM orders;
</code></pre> <p>Now we can evaluate multiple what-if scenarios in a single query using lineage edges and bit masks:</p> <h5>Extending to Many Hypotheticals</h5> <p>Each predicate can occupy a bit in a 64-bit column in <code>orders_tm</code>. We can evaluate 64 scenarios in parallel using bitwise operations:</p> <pre><code class="language-sql">
SELECT
    b.oid,
    SUM(
...
In addition, this reduces to matrix operations that can be evaluated using simple nested loops.</p> <h4>What We Get</h4> <ul><li><strong>Efficient evaluation:</strong> 64 scenarios per row can be computed in parallel.</li> <li><strong>Minimal data movement:</strong> only the lineage block and masks are needed.</li> <li><strong>Flexible analysis:</strong> supports aggregates, policy-based filters, and arbitrary hypothetical updates.</li> <li><strong>Required for explanation engines:</strong> under the hood, they evaluate many hypothetical updates to find which change explains an outlier.</li></ul> <h2 class="mt-4 mb-3">Follow-ups</h2> <ul><li><a class="link-primary">What-ifs: Sparse Encoding</a></li> <li><a class="link-primary">Lineage as a DuckDB Extension</a></li> <li><a class="link-primary">← Back to main post</a></li></ul></main>`);function _(c){var t=f(),d=s(e(t),50),i=e(d),p=e(i);a(i);var o=s(i,2),u=e(o);a(o);var l=s(o,2),h=e(l);a(l),a(d),a(t),b(()=>{n(p,"href",`${r??""}/part-b`),n(u,"href",`${r??""}/part-c`),n(h,"href",r)}),m(c,t)}export{_ as component};
