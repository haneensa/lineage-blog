...
GROUP BY output_id, code;
  </code></pre> <p>Each output row aggregates only the contributions of rows that satisfy the predicate.
    The <code>row.code</code> encoding replaces a full boolean matrix and allows <strong>linear-time evaluation</strong> across all interventions.</p> <h3>Range Predicates and Prefix Sums</h3> <p>For range predicates like <code>order.value ≤ k</code>, each row contributes to all updates whose threshold exceeds its value. Instead of expanding a dense boolean matrix, we can compute a <strong>prefix sum</strong> over sorted row codes.</p> <p>Conceptually:</p> <pre><code>result(out, k) = total(out) − Σ rows with value ≤ k</code></pre> <p>In SQL, this can be implemented using a window function:</p> <pre><code class="language-sql">
WITH agg AS (
    SELECT
...
</code></pre> <p>Here, <code>SUM(1) OVER ...</code> acts as a prefix sum over the sparse codes, computing efficiently which rows remain under each threshold <code>x</code>.
This mirrors the bitwise evaluation of multiple what-ifs from Part A.</p> <h3>Combining Equality and Range Predicates</h3> <p>More complex updates, e.g., <code>v = x AND w ≤ y</code>, can be handled by:</p> <ol><li>Partitioning rows by equality attributes</li> <li>Applying a prefix sum over the range attribute within each partition</li></ol> <p>This allows us to compute a two-dimensional space of updates without materializing a quadratic matrix.</p> <h2>Why This Matters</h2> <ul><li><strong>Scalability:</strong> reduces evaluation from O(|rows| × |updates|) to O(|rows|)</li> <li><strong>Memory efficiency:</strong> replaces dense boolean matrices with compact integer codes</li> <li><strong>SQL-native:</strong> implemented using grouping and window functions</li> <li><strong>Composable:</strong> integrates cleanly with lineage-based recomputation</li></ul> <p>Sparse encodings align naturally with <strong>explanation engines</strong> and <strong>counterfactual search</strong>.  
Each intervention is a partition of the data, and aggregates can be computed efficiently over codes, even when testing thousands of hypothetical updates.</p> <p>The following uses these concepts <a href="https://haneensa.github.io/whatif-demo/" target="_blank">DEMO</a> We reimplement of <a href="http://sirrice.github.io/files/papers/scorpion-vldb13.pdf" target="_blank">Scorpion</a> explanation engine using this approach to evaluate hypothetical updates.
